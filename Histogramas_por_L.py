...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 15:21:28 2019
...
pathdatos='/media/tony/TOSHIBAEXT/TESIS/DATOS_BINEADOS/DATOS/FESA_BINARIO/'
Lista_Nombres=sorted(os.listdir(pathdatos))
bines=np.linspace(1,7,31)+0.1
DATOS_L_hola=[]
DATOS_L_primero=[]
for i in range(len(Lista_Nombres)):
    DATOS_L_hola=np.load(pathdatos+str(Lista_Nombres[i]))[:,0]
...
DATOS_Totales=(np.load(pathdatos+str(Lista_Nombres[len(Lista_Nombres)-1]))[:,0])
...
fig.suptitle('Histogramas por L para L=1.1 para 1.8MeV',fontsize=24)
